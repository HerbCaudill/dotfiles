...
For capacity-limited baseline/target showcases, reuse one owned environment: record baseline evidence, build the target with `sync`, then `reset` to the immutable original snapshot before starting and validating the target. This avoids simultaneous 40 GB INL catalogs and prevents baseline edits from carrying into the target. Refresh is reconciliation, not a complete reverse migration; extra unused columns may remain.
