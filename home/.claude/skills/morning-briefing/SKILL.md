...
**Previous briefings** (main agent, read-only): before launching the gatherers, read only the `## Daily briefing` sections from the three most recent dated notes before today that contain one. Use their `### Open issues` and `### Next steps` sections to make a carryover checklist. Include each relevant checklist item in the source-gathering prompt most likely to verify it.
