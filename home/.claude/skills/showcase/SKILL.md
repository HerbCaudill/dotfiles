...
Record the starting branch or detached commit and relevant runtime state. Resolve the target and baseline:
...
Briefly announce the change, the source of its intent, the problem, the intended outcome, what would count as success, and the exact revisions. Choose one headline scenario and only the high-risk edge cases needed to understand it.
...
Unless project guidance requires otherwise, use the existing clean checkout sequentially: serve the baseline for stage 1, then the target for stages 2 and 3. Stop or restart affected processes and rebuild as required. Before each demonstration, verify the starting data, application health, and that the running application serves the intended revision. A checkout alone does not prove runtime freshness.
...
1. Serve the verified baseline and explain the problem in plain language.
...
Serve the verified target with equivalent starting state. Repeat the same path and meaningful action where possible. Explain what to do, what changed, what should happen, and any important edge cases.
...
Restore the starting branch or detached commit even if the walkthrough stops early. Stop showcase-only processes as appropriate and restore the original runtime when project guidance makes that safe and practical. Offer to remove showcase fixtures; delete them only with confirmation.
