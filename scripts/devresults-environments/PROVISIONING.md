...
On September 15 the helper checks passed. Live useful-data provisioning remains unperformed: no coordinated SQL/blob snapshot has been supplied, and the VM was observed with 6.86 GiB free against SQL allocations of approximately 5.64 GiB (`dev`) and 40.58 GiB (`dev-inl`), before the deployment copy and headroom. Later read-only inspection found that the virtual disk is already 1 TiB and an oversized active WinRE partition blocks C: extension. See [the partition layout evidence](INSTALLATION-EVIDENCE.md#partition-layout-findings). Capacity recovery requires a verified backup and explicit approval for maintenance of the recovery partition before extending C:; merely enlarging the virtual disk does not resolve it. A coordinated snapshot with the owners of all source writers is also still required. Existing primary/review IIS processes remain untouched. Elevated SSH and the default `MSSQLSERVER` service were observed; this does not imply a restore/runtime integration pass.
