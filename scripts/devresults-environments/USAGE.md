...
The current VM already has a 1 TiB virtual disk. A large active WinRE recovery partition immediately after C: blocks a simple C: extension. The capacity prerequisite is maintenance of the recovery partition layout with a verified backup and explicit approval, followed by C: extension and a fresh capacity check. Increasing virtual disk size alone does not resolve the layout. [The partition evidence](INSTALLATION-EVIDENCE.md#partition-layout-findings) records the exact sizes and read-only findings; no partition changes are part of `drenv` installation.
